    ...
    try {
        ...
        if (savedData) {
            ...
            if (!currentRaw && parsedData.consumedDrinks) {
                currentRaw = { day: parsedData.dayCounter || 1, drinks: parsedData.consumedDrinks };
            }
...
function startNewDay() {
    ...
    if (confirm(`Möchtest du Tag ${state.currentDay.day} wirklich abschließen?\nEin neuer, leerer Tag wird gestartet.`)) {
...
function getCategoryIcon(category) {
    const icons = {
        'Sprizz': '🍹', 'Bier vom Fass': '🍺', 'Flaschenbier': '🍺',
        'Cocktail': '🍸', 'Kaffee': '☕', 'Softdrink': '🥤',
        'Wasser': '💧', 'Wein - WEISS': '🥂', 'Wein - ROT': '🍷',
...
function renderOverview() {
    ...
    allDays.forEach(dayData => {
        if (dayData.drinks.length === 0 && dayData.day !== state.currentDay.day) return;
...
function renderDrinkList() {
    ...
    drinksData.forEach(drink => {
        ...
        `;
        li.querySelector('.add-btn').addEventListener('click', () => addDrink(drink));
